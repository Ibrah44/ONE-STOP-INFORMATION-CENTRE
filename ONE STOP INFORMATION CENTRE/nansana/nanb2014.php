...
$graph = new PHPGraphLib(600,600);
 ...
 //nansana aspects
             $data = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk"  && deadline like "2014-02%"  ');
                         $num_ro = mysql_num_rows($data);

                     //echo $num_ro."total"."<br/>";
					 
                     $result2n = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Pending"  && deadline like "2014-02%" ');
		...
		$result3 = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Accomplished" && deadline like "2014-02%" ');
...
$graph->addData($data);
$graph->setTitle('Nansana Town Council Duty Anaysis For February 2014(%)');
