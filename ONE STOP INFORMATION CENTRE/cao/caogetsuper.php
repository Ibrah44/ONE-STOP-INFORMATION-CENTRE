...
echo	'<tr>
		 ...
		 <td><b>Inspection Report</b></td>
		 <td><b>Inspector</b></td>
		 </tr/>';
//echo '(projectId,projectName,projectDetail,dateOfSubmission,submitedBy,document,status)'	
	
echo	'</tr>';
	$getinspects=mysqli_query($conn,'select * from inspect order by inspectionDate desc');
	
	
	while($shwinp= mysqli_fetch_array($getinspects))
	   {
	...
	echo ' 
	  ...
	  <td> <a href = '.$shwinp["inspectionProof"].' />Download</a></td>
	  <td>'.$shwinp["doneBy"].'</td>
	...
	echo '<center>';
    echo '<a href="caoallocateDuty.php"><input type="button" value="Previous" class="btn btn-primary " style="width:110px;"></a>';
	echo '&nbsp;&nbsp;&nbsp';        
