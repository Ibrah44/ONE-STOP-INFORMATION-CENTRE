...
$graph = new PHPGraphLib(500,400);
 ...
 //nansana aspects
             $data = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk"  && deadline like "2015%" ');
                         $num_ro = mysql_num_rows($data);

                    // echo $num_ro."total"."<br/>";
					 
                     $result2n = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Pending"  && deadline like "2015%" ');
						 ...
						 //% of pending
							 ...
							 // echo $try2."<br/>";
							    $result3 = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Accomplished" && deadline like "2015%" ');
			...
			$resultf = mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && deadline like "2015%" ');
                 
 				 $num_rowf = mysql_num_rows($resultf);
				 
				 //echo $num_rowf."total duties"."<br/>";
                      
					$result2f = mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && status="Pending" && deadline like "2015%"');
						...
						//echo $num_rof."pending"."<br/>";
						 //% of pending
							  ...
							  //echo $tryf."<br/>";
							    ...
							    $result3f= mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && status="Accomplished" && deadline like "2015%" ');
...
$graph->setTitle('Town Council Duty Anaysis For 2015');
