...
$graph = new PHPGraphLib(500,400);
 ...
 //nansana aspects 2013
             $data = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && deadline like "2013%"  ');
                         $num_ro = mysql_num_rows($data);

                    // echo $num_ro."total"."<br/>";
					 
                     $result2n = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Pending"  && deadline like "2013%" ');
						 ...
						 //% of pending
							 ...
							 // echo $try2."<br/>";
							    $result3 = mysql_query('SELECT * FROM duty where assignedTo="Nansana Town Clerk" && status="Accomplished" && deadline like "2013%" ');
			...
			$resultf = mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && deadline like "2013%" ');
                 
 				 $num_rowf = mysql_num_rows($resultf);
				 
				 //echo $num_rowf."total duties"."<br/>";
                      
					$result2f = mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && status="Pending" && deadline like "2013%"');
						...
						//echo $num_rof."pending"."<br/>";
						 //% of pending
							  ...
							  //echo $tryf."<br/>";
							    ...
							    $result3f= mysql_query('SELECT * FROM duty where assignedTo="kira Town Clerk" && status="Accomplished"  && deadline like "2013%" ');
